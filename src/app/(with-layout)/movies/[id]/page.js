...
useEffect(() => {
  ...
  let handleBookNow = async (e) => {
    ...
    setBookingExists("");
    setError("")
     if (mybookings.some(booking => booking.seatNo === myseat && booking.movieId === movie?._id)) {
    setBookingExists("You have already booked this seat.");
    return;
  }
  if(bookings.some((x)=>x.seatNo===myseat&&bookings.movieId===movie?._id)){
 ...
 await fetchBookings()
    ...
    <>
      <section>
        ...
        <div className='lg:me-14 lg:mt-8 mt-60 '>
          <h1 className='text-white text-4xl lg:text-end'>{movie.original_title}</h1>
          <p className='text-gray-400 lg:text-end mt-2'>Release Date: {movie.release_date}</p>
          <h3 className='text-white text-3xl  lg:text-end mt-2'>${movie.price}</h3>
     ...
     <div className='mt-20'>
     <h1 className='text-center text-white text-5xl  font-semibold'>Book Seats</h1>
      <div className='grid lg:grid-cols-6 md:grid-cols-3 grid-cols-2 gap-5 mt-20 items-center justify-center  '>
        {seats.map((x,index)=><div key={index} className='text-center'> <button   className={`btn border ${
          mybookings.some((booking) => booking.seatNo ===x.seat&&booking.email===session?.user?.email&&booking.movieId===movie?._id)
            ? 'bg-green-500 text-white'
            : 'border-yellow-500 text-black'
        }     ${
          bookings.some((booking) => booking.seatNo ===x.seat&&booking.movieId===movie?._id&&booking.email!==session?.user?.email)
  ...
  <div>
  <h3 className="font-bold text-lg">Book {movie?.original_title}</h3>
...
{bookingExists&& <p className='text-red-500 text-sm mt-3'>{bookingExists}</p>}
    <div className="modal-action">
      <button 

       className="btn bg-yellow-500" onClick={handleBookNow}>Confirm Booking</button>
    </div>
  </form>
</dialog>
