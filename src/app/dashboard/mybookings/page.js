...
const MyBookings = () => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      if (status === 'authenticated' && session?.user?.email) {
        try {
          const response = await fetch(`${process.env.NEXT_PUBLIC_DEV_API_URL}/api/mybooking/${session.user?.email}`);
  ...
  const handleCancelBooking = async (id) => {
    const response = await fetch(`${process.env.NEXT_PUBLIC_DEV_API_URL}/api/mybookings/${id}`, {
            method: 'DELETE'
          });
...
let handleRequestForRefund= async(id)=>{
  try {
    await fetch(`${process.env.NEXT_PUBLIC_DEV_API_URL}/api/mybookings/${id}`, {
      method: 'PATCH',
      headers: {
